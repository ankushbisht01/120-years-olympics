...
class get_data:
    ...
    def age_vs_medal(self):
        ...
        PREFIX oly: <https://w3id.org/def/OlympicsOntology#>
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX foaf: <http://xmlns.com/foaf/0.1/>
        PREFIX dbp: <http://dbpedia.org/property/>
        PREFIX na: <http://wallscope.co.uk/resource/olympics/athlete/>
        SELECT ?age (count(?medalname) as ?totalmedal) 
        WHERE {
        ?instance oly:Athelete ?athelete ;
                oly:Medal ?medal .
        ?athelete oly:hasAge ?age .
  		?medal oly:hasMedal ?medalname .
        } 
        GROUP BY ?age
        ORDER BY ?age
    ...
    def ath_age_medals(self,name):
            ...
            PREFIX dbp: <http://dbpedia.org/property/>
            PREFIX na: <http://wallscope.co.uk/resource/olympics/athlete/>
            SELECT  (COUNT(?medalname) As ?noOfMedals)
            WHERE {
            ?instance oly:Athelete ?athelete ;
                    oly:Medal ?medal .
            ?athelete oly:hasName ?name .
  			?athelete oly:hasAge ?age .
  			?medal oly:hasMedal ?medalname
    ...
    def get_game_name(self,name_of_athlete):
        ...
        ?instance oly:Athelete ?athelete ;
                    oly:Sport ?sport .
  			?sport oly:hasSportName ?sportname .
            ?athelete oly:hasName ?name .
            filter contains(?name , \""""+name_of_athlete+"""\")       
    ...
    def top_male_athlete(self):
        ...
        PREFIX na: <https://w3id.org/def/OlympicsOntology#>
        SELECT ?name  (COUNT(?medalname) As ?noOfMedals)
        WHERE {
        ?instance oly:Athelete ?athlete ;
                oly:Medal ?medal .
        ?athlete oly:hasName ?name ;
                oly:hasGender ?gender .
  		?gender rdfs:label ?gendername .
  		?medal oly:hasMedal ?medalname .
        filter contains(?gendername,"male")
    ...
    def top_female_athlete(self):
        ...
        ?athlete oly:hasName ?name ;
  		...
  		?gender rdfs:label ?gendername .
  		?medal oly:hasMedal ?medalname .
        filter contains(?gendername,"female")
        }
    ...
    def get_country_stat(self,country_name):
        ...
        ?athlete oly:hasName ?name ;
                    oly:hasAge  ?age ;
                oly:hasHeight ?height ;
                oly:hasWeight ?weight ;
                oly:hasNationality ?nationality .
  	    ?nationality oly:countryName ?countryname
          filter contains(?countryname , \""""+country_name+"""\")
    ...
    def stats_other_country(self,name_of_the_excluding_country):
        ...
        ?athlete oly:hasName ?name ;
          ...
          minus {?nationality oly:countryName \""""+name_of_the_excluding_country+"""\" . }
    ...
    def top10_medals(self):
        ...
        where {
        ?instance oly:Athelete ?athlete.
        OPTIONAL { ?instance oly:Medal ?medal . }
        ?athlete rdfs:label ?names .
        }
        group by ?names
    ...
    def search(self,name):
        ...
        select ?names (Count(?medal) as ?totalmedal)
        where {
        ?instance oly:Athelete ath:"""+name+""" .
        OPTIONAL { ?instance oly:Medal ?medal . }
    ...
    def insert_data(self,df:pd.DataFrame):
        ...
        for i in range(len(df)):
            ...
            query = query + f""" 
                    ...
                    <http://wallscope.co.uk/resource/olympics/instance/{game_uri}/{data['Year']}/{data['Season']}/{uri_name}> oly:Athelete <http://wallscope.co.uk/resource/olympics/athlete/{uri_name}>;
            ...
            try:
                ...
                if isinstance(first_col['Medal'], str):
                    query = query + (f'''<http://wallscope.co.uk/resource/olympics/medal/{data["Medal"]}> rdfs:label "{data["Medal"]}" .
                            <http://wallscope.co.uk/resource/olympics/instance/{game_uri}/{data['Year']}/{data['Season']}/{uri_name}> oly:Medal <http://wallscope.co.uk/resource/olympics/medal/{data['Medal']}> .''')
